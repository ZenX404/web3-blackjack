...
export default function Page() {
  ...
  async function handleSendTx() {
    ...
    /**
     * 先使用 simulateContract 模拟合约调用，然后再使用 writeContract 发送真实交易请求，这是一种很好的实践。让我详细解释这样做的原因：
      模拟合约调用的好处
      预先验证交易
        模拟调用可以检查交易是否会成功，而不需要实际发送交易到区块链
        这样可以避免因为交易失败而浪费 gas 费用
      错误检测和调试
        如果合约调用会失败，模拟调用会返回具体的错误信息
        这比直接发送交易后才发现失败要友好得多，因为区块链上的错误信息通常不够详细
      估算 gas 费用
        模拟调用可以帮助估算实际交易所需的 gas 费用
        这使用户可以提前了解交易成本，避免意外的高额费用
      检查状态变化
        模拟调用可以预览交易将如何改变合约状态
        这对于复杂的交易尤其有用，可以确保交易的结果符合预期
      提高用户体验
        通过先模拟调用，可以立即向用户提供反馈
        如果模拟失败，可以显示具体错误，而不是让用户等待交易被区块链拒绝

      先模拟合约调用再发送真实交易请求是一种最佳实践，它可以：
        提前发现错误，避免浪费 gas 费
        提高用户体验，提供更好的错误反馈
        对于涉及资金或有价值资产（如 NFT）的操作，先模拟可以增加一层安全保障。增加交易的安全性和可靠性。
        帮助用户了解交易的潜在影响和成本
     */

    /**
     * 这段代码的执行流程是：
        首先使用 publicClient.simulateContract() 模拟调用合约
        如果模拟成功（没有抛出异常），则继续执行
        然后使用 walletClient.writeContract() 发送真实交易
        获取交易哈希 txHash 用于后续跟踪交易状态
        如果模拟调用失败，会抛出异常，前端可以捕获这个异常并向用户显示错误信息，避免用户发送必然会失败的交易。
     */
    // 模拟合约调用
